...
public class PlayerClockTest extends InstrumentationTestCase {
	...
	public PlayerClock createPlayerClock() {
		...
		timeAmountPerEpoch.put(FakeEpoch.ONE, new TimeAmount(30000L));
		timeAmountPerEpoch.put(FakeEpoch.TWO, new TimeAmount(30000L));
		return new PlayerClock(new FakePlayerId(), new TimeAmount(60000L), new TimeAmount(30000L),
				timeAmountPerEpoch, new TimeAmount(10000L), new TimeAmount(10000L));
	}

	public void testGetRemainingTime() throws Exception {
		PlayerClock playerClock = createPlayerClock();
		assertEquals(new TimeAmount(60000L), playerClock.getRemainingReserveTime(
				new TimeInstant(0L)));
	}

	public void testGetRemainingUpkeepTime() throws Exception {
		PlayerClock playerClock = createPlayerClock();
		assertEquals(new TimeAmount(0L), playerClock.getRemainingUpkeepTime(
				new TimeInstant(1000L)));
		playerClock.upkeep(new TimeInstant(2000L));
		assertEquals(new TimeAmount(29000L), playerClock.getRemainingUpkeepTime(
				new TimeInstant(3000L)));
		assertEquals(new TimeAmount(28000L), playerClock.getRemainingUpkeepTime(
				new TimeInstant(4000L)));
		// upkeep time exceeded
		assertEquals(new TimeAmount(0L), playerClock.getRemainingUpkeepTime(
				new TimeInstant(65000L)));
	}
	...
	public void testGetRemainingTimeAfterStart() throws Exception {
		...
		assertEquals(new TimeAmount(-1000L), playerClock.getRemainingReserveTime(
				new TimeInstant(62000L)));
	}
	...
	public void testGetRemainingTimeAfterStop() throws Exception {
		...
		assertEquals(new TimeAmount(59000L), playerClock.getRemainingReserveTime(
				new TimeInstant(3000L)));
		// time is still not decreasing
		assertEquals(new TimeAmount(59000L), playerClock.getRemainingReserveTime(
				new TimeInstant(100000L)));
	}

	public void testAddReserveTime() throws Exception {
		PlayerClock playerClock = createPlayerClock();
		playerClock.addReserveTime(new TimeInstant(0L), new TimeAmount(1000L));
		assertEquals(new TimeAmount(61000L), playerClock.getRemainingReserveTime(
				new TimeInstant(1000L)));
	}
	...
	public void testAddTurnBonusTime() throws Exception {
		PlayerClock playerClock = createPlayerClock();
		playerClock.addTurnBonusTime(new TimeInstant(0L), FakeEpoch.ONE);
		assertEquals(new TimeAmount(90000L), playerClock.getRemainingReserveTime(
				new TimeInstant(1000L)));
	}
	...
	public void testUpkeepStartLong() throws Exception {
		...
		playerClock.upkeep(new TimeInstant(0L));
		playerClock.start(new TimeInstant(10000L));
		assertEquals(new TimeAmount(50000L), playerClock.getRemainingReserveTime(
	...
	public void testStartUpkeepLong() throws Exception {
		...
		playerClock.start(new TimeInstant(0L));
		playerClock.upkeep(new TimeInstant(10000L));
		assertEquals(new TimeAmount(40000L), playerClock.getRemainingReserveTime(
				new TimeInstant(50000L)));
	}
	...
	public void testGetRemainingUpkeepTimeAfterRepeatedStart() throws Exception {
		...
		assertEquals(new TimeAmount(58000L), playerClock.getRemainingReserveTime(
				new TimeInstant(33000L)));
	}
	...
	public void testUpkeepStartStopShort() throws Exception {
		...
		assertEquals(new TimeAmount(60000L), playerClock.getRemainingReserveTime(
				new TimeInstant(3000L)));
		assertEquals(new TimeAmount(26000L), playerClock.getRemainingUpkeepTime(
				new TimeInstant(4000L)));
	}
	...
	public void testStartUpkeepStopShort() throws Exception {
		...
		assertEquals(new TimeAmount(59000L), playerClock.getRemainingReserveTime(
				new TimeInstant(3000L)));
		assertEquals(new TimeAmount(27000L), playerClock.getRemainingUpkeepTime(
				new TimeInstant(4000L)));
	}
	...
	public void testStartUpkeepPauseResume() throws Exception {
		...
		assertEquals(new TimeAmount(1000L), playerClock.getRemainingUpkeepTime(
				new TimeInstant(88000L)));
		assertEquals(new TimeAmount(58000L), playerClock.getRemainingReserveTime(
				new TimeInstant(90000L)));
	}
	...
	public void testStartPauseResume() throws Exception {
		...
		playerClock.resume(new TimeInstant(60000L));
		assertEquals(new TimeAmount(0L), playerClock.getRemainingUpkeepTime(
				new TimeInstant(70000L)));
		assertEquals(new TimeAmount(28000L), playerClock.getRemainingReserveTime(
				new TimeInstant(90000L)));
	}
	...
	public void testNegativeUpkeepTimeNotAllowed() throws Exception {
		try {
			new PlayerClock(new FakePlayerId(), new TimeAmount(60000), new TimeAmount(-5000),
					FakePlayerClock.createTimeAmountPerEpoch(), new TimeAmount(20000),
	...
	public void testNegativeTurnBonusTimeNotAllowed() throws Exception {
		try {
			TimeAmountPerEpoch timeAmountPerEpoch = new TimeAmountPerEpoch();
			timeAmountPerEpoch.put(FakeEpoch.ONE, new TimeAmount(20000));
			timeAmountPerEpoch.put(FakeEpoch.TWO, new TimeAmount(-20000));
			new PlayerClock(new FakePlayerId(), new TimeAmount(60000), new TimeAmount(5000),
					timeAmountPerEpoch, new TimeAmount(-20000), new TimeAmount(10000L));
			Assert.fail("Should have thrown IllegalArgumentException.");
		} catch (IllegalArgumentException e) {
			// success
		}
